(ns perf.capture-test
  (:require [clojure.test :refer [deftest is testing]]
            [perf.capture :as capture]))

(deftest emit-lands-in-jfr-stream
  (testing "a custom perf event (EventFactory) is captured in a JFR stream with typed fields"
    (let [rs  (jdk.jfr.consumer.RecordingStream.)
          got (atom nil)]
      (.enable rs "perf.Test")
      (.onEvent rs "perf.Test"
                (reify java.util.function.Consumer
                  (accept [_ e] (reset! got {:factor (.getDouble e "factor")
                                             :name (.getString e "name")
                                             :reps (.getLong e "reps")}))))
      (.startAsync rs)
      (Thread/sleep 300)
      (capture/emit "perf.Test" {:factor 8.28 :name "dot" :reps 1000000})
      (loop [i 0] (when (and (nil? @got) (< i 60)) (Thread/sleep 100) (recur (inc i))))
      (.close rs)
      (is (= {:factor 8.28 :name "dot" :reps 1000000} @got)))))
