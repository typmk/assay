...
;;
;;   assay.control/trace!   [[6] 12]                 flat, per-var, timing
;;                                                  in a separate map
...
;; version of engineering. `assay.trace` is a bridge to Sayid instead.
...
(defn pending
  ...
  ([n]
   (if-let [e (some-> @dbg :pending deref)]
     (let [t (.thread e)]
       (cond-> {:assay.dbg/thread (.name t)
                :assay.dbg/at (condp instance? e
         ...
         (assoc :assay.dbg/exception (.name (.referenceType (.exception e))))))
     :nothing-suspended)))
...
(defn- object-ref [vm value]
  (carrier value)
  (let [rt (first (.classesByName vm "assay.control.Carrier"))]
    (when rt
