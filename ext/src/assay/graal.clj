(ns assay.graal
  ...
  assay.jvmci reads what types DID flow (the JIT's runtime profile). This is
  ...
  on a stock JVM, so like assay.jvmci it is source-only, loaded on demand on
...
(defn- stamp->data
  ...
  (cond
    ...
    (let [s ^IntegerStamp s]
      #:assay.graal{:kind :int :bits (.getBits s) :lo (.lowerBound s) :hi (.upperBound s)})
    (instance? FloatStamp s)
    (let [s ^FloatStamp s]
      #:assay.graal{:kind :float :bits (.getBits s) :lo (.lowerBound s) :hi (.upperBound s)
                   :non-nan (.isNonNaN s)})
    (instance? AbstractObjectStamp s)
    (let [s ^AbstractObjectStamp s]
      #:assay.graal{:kind :object
                   :type (some-> (.type s) .toJavaName)
                   :exact (.isExactType s)
                   :non-null (.nonNull s)})
    (instance? VoidStamp s) #:assay.graal{:kind :void}
    :else #:assay.graal{:kind :other :repr (str s)}))
       ...
       (keep (fn [n]
               (when (instance? ValueNode n)
                 (let [st (stamp->data (.stamp ^ValueNode n NodeView/DEFAULT))]
                   (assoc st
                          :assay.graal/node (.getSimpleName (class n))
                          :assay.graal/id (.getId ^jdk.graal.compiler.graph.Node n))))))
       vec))
...
(defn stamps-of
  ...
  'a non-null Object', and that itself is the point assay keeps making."
  [f]
  (let [{:keys [^MetaAccessProvider meta]} (ctx-bundle)
