(ns assay.jvmci
  ...
  straight from JVMCI. This is the one thing assay otherwise cannot give and
  ...
  caller drives it, this reads it. Same shape as `assay.code/native`: you ran
...
(defn profile
  ...
  first.
    ...
    (assay.jvmci/profile my-fn)        ; read what the JIT saw"
  [f]
  (let [ma (meta-access)]
    (vec
     (for [^java.lang.reflect.Method jm (fn-methods f)
       ...
       (cond-> #:assay.jvmci{:method (.getName jm) :bci bci}
         (pos? ec)   (assoc :assay.jvmci/count ec)
         (>= bp 0.0) (assoc :assay.jvmci/branch-taken-prob bp)
         tp          (assoc :assay.jvmci/types
...
;; call" in assay.code/kind-order — a rank-3 statement about how the
...
(defn notes
  ...
  in the same shape assay.code emits, so it ranks, tiers and muffles
  ...
  (let [w (type-profile-width)]
    (assay.code/report
     (for [e (profile f)
           :let [ts (remove #(= :other (first %)) (:assay.jvmci/types e))
                 n  (count ts)]
           :when (> n inline-cache-limit)]
       #:assay.note{:code :assay.note/megamorphic
                   :severity (assay.code/severity :assay.note/megamorphic)
                   :span {:assay/file nil
                          :assay/line nil
                          :assay/col (:assay.jvmci/bci e)}
                   :message (format "%s receiver types at %s bci %s (%s of %s recorded%s)"
                                    n (:assay.jvmci/method e) (:assay.jvmci/bci e) n w
