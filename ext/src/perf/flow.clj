(ns perf.flow
  ...
  THE DIVISION OF LABOUR
    perf       answers WHERE the cost is — which fn allocates, blocks,
  ...
  configuration, not a library you call. So `perf` cannot depend on it,
  ...
  What CAN be bridged is the handoff: perf produces a site
  (:perf/site {:perf/fn \"user/handle-request\"}); FlowStorm instruments
...
(defn- site->var-sym
  "perf site -> the var symbol FlowStorm wants.
...
(defn instrument-site!
  "Instrument the var behind a perf SITE (or a whole observation/row).

  The handoff: profiling found the hot function; this records its values.

    (->> (perf.repl/allocation) first perf.flow/instrument-site!)
  ...
  (let [site (or (:perf/site site-or-row) site-or-row)
    ...
    (cond
      (not (available?)) {:perf.flow/error :not-on-classpath
                          :perf.flow/remedy "add com.github.flow-storm/flow-storm-dbg"}
      (nil? sym) {:perf.flow/error :no-var-in-site :perf/site site}
...
(defn instrument-hot!
  "Instrument the top N sites from a perf result — the whole workflow in
...
(defn ui!
  ...
  []
  (if-let [f (fs 'local-connect)] (f {}) {:perf.flow/error :not-on-classpath}))
