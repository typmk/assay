(ns perf.remote
  ...
  WHY THIS EXISTS SEPARATELY FROM perf.control:

  perf.control self-attaches — the debugger and the target are one JVM.
  ...
  trick (perf.control/object-ref), but you cannot trick your way to
  calling a method.
  ...
  USAGE — two JVMs:
    ...
    target$  clj -M:debug:perf ...            ; jdwp on 127.0.0.1:8899
    dbg$     clj -M:perf-ext
             (require '[perf.remote :as rem])
...
(defn attach
  ...
  ([host port]
   (let [vmm (com.sun.jdi.Bootstrap/virtualMachineManager)
     ...
     {:perf.remote/vm (.attach ^com.sun.jdi.connect.AttachingConnector conn args)
      :perf.remote/pending (atom nil)
      :perf.remote/host host
      :perf.remote/port port})))

(defn arm!
  "Suspend the target when EXCEPTION-CLASS is thrown, pre-unwind."
  [{:keys [:perf.remote/vm :perf.remote/pending]} exception-class]
...
(defn pending
  ...
  ([conn] (pending conn 8))
  ([{:keys [:perf.remote/pending]} n]
   (if-let [e @pending]
     (let [t (.thread e)]
       {:perf.dbg/exception (.name (.referenceType (.exception e)))
        :perf.dbg/thread (.name t)
        :perf.dbg/frames (vec (for [^com.sun.jdi.StackFrame f (take n (.frames t))]
                                #:perf.dbg{:fn (fname f)
...
(defn local
  ...
  ([{:keys [:perf.remote/pending]} name idx]
   (when-let [e @pending]
...
(defn eval-str
  ...
  ([{:keys [:perf.remote/vm :perf.remote/pending] :as conn} name idx]
   (when-let [e @pending]
     (let [t (.thread e)
           v (local conn name idx)]
       (cond
         (nil? v) {:perf.dbg/error :no-such-local :perf.dbg/name name}
...
(defn box
  ...
  self-attach could not do — the whole reason perf.control needed the
  Carrier field-read trick."
  [{:keys [:perf.remote/vm :perf.remote/pending]} v]
  (when-let [e @pending]
    (let [t (.thread e)
          [cls sig prim]
...
(defn- register-boxes!
  ...
  perf.control fixes this by calling Class/forName locally — it can,
  ...
  [{:keys [:perf.remote/vm]} ^com.sun.jdi.ReferenceType rt ^com.sun.jdi.ThreadReference t]
  (try
...
(defn use-value!
  ...
  [{:keys [:perf.remote/vm :perf.remote/pending] :as conn} fn-prefix value]
  (if-let [e @pending]
    (let [t (.thread e)
          ...
          idx (find-idx)]
      (if-not idx
        {:perf.dbg/error :no-frame-matching :perf.dbg/prefix fn-prefix}
        ...
        (let [rt    (.declaringType (.location (nth (vec (.frames t)) idx)))
          ...
          (let [mirrored (when (some? value) (box conn value))
            ...
            (.resume t)
            (reset! pending nil)
            #:perf.dbg{:restarted where :returned value}))))
    :nothing-suspended))

(defn detach [{:keys [:perf.remote/vm]}] (.dispose vm) :detached)
