...
`assay.control` had a hand-rolled Var-rebinding tracer. **It has been
...
`assay.trace` is now a bridge: profile finds the site, Sayid records its
values. Keeping a worse reimplementation because I wrote it would have
been the sunk-cost version of engineering.

What `assay` does that Sayid does not: JFR-sourced allocation, blocking
...
this: Tufte measures what you instrument, `assay` samples what the JVM
...
`assay` calls it for flamegraphs rather than reimplementing it. Its
...
The clojure-goes-fast family. `assay` depends on the first three.
...
`assay.control`, and on a different axis from JFR sampling.
`assay.flow` is a 60-line bridge and deliberately nothing more.

### CDT — https://github.com/GeorgeJahad/cdt

**The prior art for `assay.control` and `assay.remote`, which I missed on
...
swank. The ideas did not die with the code, and two of them were adopted:
  ...
  * **frame locals** on the suspended stack (`assay.control/pending`)
  * **the separate-process architecture** (`assay.remote`), which is not a
    ...
    same process and it deadlocks. That single fact is why `assay.control`
...
**Prior art for `assay.code/notes`, found only because Apollo asked "surely
...
that job, and `assay.diagnose/scan` is the part of this that genuinely
duplicates it.

### clj-java-decompiler / Clojure Goes Fast

The blog makes the argument `assay.code/types` acts on — that checking
...
**Prior art for the half of `assay.control/restarts!` that should never
...
`assay.native` (three safety functions, not FFI plumbing) — but it is
...
- **Portal / Reveal / REBL** — data browsers. `assay` implements
...
2. **`:assay/site` vs `:assay/via`.** Recording both the nearest frame you
