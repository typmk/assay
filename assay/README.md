...
```clojure
(require '[assay.repl :as r])

(r/start!)                    ; ... run a workload ...
(r/allocation)
;; => [#:assay{:fn "app/handle-request", :line 6, :n 403} ...]

(r/summary)
;; => #:assay{:samples .. :verdict :assay.verdict/floor-rising
;;           :assay.heap/floor-mb .. :top-alloc [...] :top-blocked [...]}
```
...
```clojure
(require '[assay.query :as q] '[assay.capture :as capture])
...
that the oldest half is dropped, `:assay/dropped` is non-zero and
`summary` reports `:assay/coverage :assay.coverage/partial`. Counts stay
...
(assay/summary (edn/read-string ...))  ; analysed anywhere, later
```
...
(ask conn "(do (require '[assay.repl :as r]) (r/start!))")
(ask conn "(first (assay.repl/allocation))")
;; => #:assay{:fn "user/handle-request", :line 6, :n 403}
(ask conn "(assay.repl/mark!)")   ; ... later ...
(ask conn "(take 2 (assay.repl/growth))")
;; => ({:assay.class/name "clojure.lang.PersistentArrayMap"
;;      :assay/d-instances 37953 :assay/d-bytes 1214496})
```
...
;; => [#:assay.note{:code :assay.note/reflection :span {...} :message "..." :took ... :refused ...}
;;     #:assay.note{:code :assay.note/boxed-math :span {...} :message "..." :took ... :refused ...}
;;     #:assay.note{:code :assay.note/boxed-math :span {...} ...}]
```
...
Observations are `Navigable`. Nav a `:assay/site` or a stack frame and you
get the var; a var datafies to its source location.

```clojure
(clojure.datafy/nav o :assay/site (:assay/site o))
;; => #:assay{:var user/allocs, :file "...", :line 4, :arglists ([n])}
```
...
**Attribution.** Each observation carries `:assay/site` *and* `:assay/via`.
...
**Leaks are a floor, not a peak.** `:assay/verdict` compares the *minimum*
...
`assay-ext` is a separate artifact, because none of it is needed to
observe a running JVM:

- `assay.diagnose` — compile-time cost notes (boxing, reflection) as
  structured diagnostics with machine-applicable fixes, rustc-shaped
- `assay.control` — live Var tracing; JDI restarts that catch a failure
  pre-unwind and resume it with a value
- `assay.native` — FFM safety: prevent / recover / isolate
- `assay.flow` — a one-function bridge to FlowStorm

They are the ext AOT classes that would otherwise load on every JVM start. `-M:assay-ext` when you want them.
