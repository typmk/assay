(ns assay.forms
  ...
  Both oracles are the hardened ones the red-team's teardown forced:
    ...
    * COST is `assay.measure/weigh`, which rotates args through a jittered
  ...
  THE MODEL — two oracles and three views, nothing else:
    ...
    COST oracle     `assay.measure/weigh`  which is cheaper, by how much,
  ...
  (:require [assay.measure :as measure]
...
(defn- spread-val
  ...
  rank 2, closed only by `assay.range` on the arithmetic subset."
  [x k n]
  (cond
    (number? x)
...
;; thing SBCL's type system proves and assay could previously only sample.
...
(defn equivalent?
  ...
  ([form candidate args n]
   (if-let [p (prove form candidate)]
     #:assay.forms{:equivalent (= p :proven-equal)
                  :rank :assay.rank/proven
                  :basis :assay.forms/algebraic
                  :samples 0}
...
(defn- sample-equiv
  ...
  ([form candidate args n]
   (let [f (eval form) g (eval candidate)
     ...
     #:assay.forms{:equivalent (every? agree? pool)
                  :samples (count pool)
                  :rank :assay.rank/observed})))
...
(defn classify
  ...
  ([form candidate args {:keys [samples] :or {samples 40}}]
   (let [eq (equivalent? form candidate args samples)
         equiv? (:assay.forms/equivalent eq)
         ...
         bytes  (:assay.weigh/bytes-saved w)
         ...
         ;; we picked.
         cheaper? (or (= (:assay.weigh/verdict w) :assay.weigh/alternative-is-faster)
                      (and bytes (pos? bytes)))]
     #:assay.forms{:candidate candidate
                  :equivalent equiv?
                  :samples (:assay.forms/samples eq)
                  ...
                  :rank (:assay.forms/rank eq)
                  :factor factor
                  :bytes-saved bytes
                  :kind (cond (not equiv?) :assay.forms/differing-outcome
                              cheaper?      :assay.forms/cheaper-synonym
                              :else         :assay.forms/synonym)})))
...
(defn alternatives
  ...
  ([form candidates args opts]
   (let [rows (mapv #(classify form % args opts) candidates)]
     (vec (sort-by (fn [r] [(if (:assay.forms/equivalent r) 0 1)
                            (- (or (:assay.forms/factor r) 0))])
                   rows)))))
