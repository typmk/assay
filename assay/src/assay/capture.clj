...
(def event-kinds
  {"jdk.ObjectAllocationSample" :assay.kind/alloc
   "jdk.JavaMonitorEnter"       :assay.kind/block
   "jdk.ThreadPark"             :assay.kind/block
   "jdk.JavaMonitorWait"        :assay.kind/block
   "jdk.Deoptimization"         :assay.kind/deopt})

;; The rest of assay speaks qualified keywords; `start` should too. JFR's
...
;; :assay.kind/* keyword, or the JFR string itself; normalise to the string
...
(defn- ->event
  ...
  :wait :monitor :deopt), or the datafy'd :assay.kind/* keyword."
  [t]
  (cond
    (string? t) t
...
(defn- attrs-for
  ...
  (case kind
    :assay.kind/alloc
    {:assay.alloc/class (try (some-> ^jdk.jfr.consumer.RecordedClass (.getValue e "objectClass") .getName)
                            (catch Throwable _ nil))
     :assay.alloc/weight (try (.getLong e "weight") (catch Throwable _ nil))}
    :assay.kind/block
    {:assay.block/duration-ns (try (.toNanos (.getDuration e)) (catch Throwable _ nil))}
    :assay.kind/deopt
    {:assay.deopt/reason (try (.getString e "reason") (catch Throwable _ nil))
     :assay.deopt/action (try (.getString e "action") (catch Throwable _ nil))}
    {}))
                  ...
                  (when @run
                    (let [used (- (.totalMemory rt) (.freeMemory rt))
                          ...
                          gcn (reduce (fn [^long a ^java.lang.management.GarbageCollectorMXBean g]
                                      ...
                                      0 gcs)]
                      (swap! ring (fn [q]
                                    (let [q (conj q #:assay{:t (System/currentTimeMillis)
                                                           :heap used
...
(defn describe [r]
  (let [d (observations-dropped r)]
    (format "#assay/recorder{:observations %d%s :samples %d :running? %s}"
...
(defn observations
  "The facts recorded so far. THE input to assay.query.

  Accepts a live RECORDER or a SNAPSHOT value: snapshot stores the same
  facts under :assay/observations, and a caller holding a snapshot
  naturally reaches for (observations snap). Both feed query."
  [recorder-or-snapshot]
  (or (get recorder-or-snapshot :assay/observations)
...
(defn snapshot
  ...
  [recorder]
  {:assay/started (:started recorder)
   :assay/stopped (:stopped recorder)
   :assay/at (System/currentTimeMillis)
   :assay/period-ms (period recorder)
   :assay/observations (observations recorder)
   :assay/dropped (observations-dropped recorder)
   :assay/samples (samples recorder)})
...
;; ── emit: assay's OWN measurements into the JFR stream ─────────────
