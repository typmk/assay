(ns assay.model
  ...
  An observation:
    ...
    {:assay/kind   :assay.kind/alloc
     :assay/t      1784778161749
     :assay/thread \"main\"
     :assay/site   {:assay/fn \"user/allocs\" :assay/line 4}
     :assay/via    {:assay/fn \"clojure.core/vec\" :assay/line 379}
     :assay/stack  [{:assay/depth 0 :assay/fn ... :assay/line ...} ...]
     :assay.alloc/class \"java.lang.Long\"
     :assay.alloc/weight 12345}
  ...
  :assay/site vs :assay/via is load-bearing: a megamorphic Clojure call
...
(def Site
  [:map
   [:assay/fn {:optional true} :string]
   [:assay/line {:optional true} :int]])

(def Observation
  "One fact. Everything observable reduces to this."
  [:map
   [:assay/kind [:enum :assay.kind/alloc :assay.kind/block :assay.kind/deopt]]
   [:assay/t :int]
   [:assay/thread {:optional true} [:maybe :string]]
   [:assay/site Site]
   [:assay/via Site]
   [:assay/stack [:sequential Frame]]])
...
(defn cache-stats
  ...
  []
  #:assay.cache{:demunge (.size demunge-cache)
               :own (.size own-cache)
               :max cache-max})
        ...
        (if (== i n)
          ...
          (let [^jdk.jfr.consumer.RecordedFrame f (.get fs i)]
            (recur (unchecked-inc i)
                   (conj! out {:assay/depth i
                               :assay/fn (demunge (.getName (.getType (.getMethod f))))
                               :assay/line (.getLineNumber f)}))))))))
...
(defn- site-ref
  ...
  [f]
  (if f
    {:assay/fn (:assay/fn f) :assay/line (:assay/line f)}
    {}))
...
(defn observation
  ...
  (let [fs (frames (.getStackTrace e))]
    (merge {:assay/kind kind
            :assay/t (.toEpochMilli (.getStartTime e))
            :assay/thread (some-> (.getThread e) .getJavaName)
            ...
            :assay/site (site-ref (site fs))
            :assay/via (site-ref (first fs))
            :assay/stack fs}
           attrs)))
...
(defn nav-frame [frame]
  (or (some-> (resolve-frame (:assay/fn frame)) d/datafy) frame))

(extend-protocol p/Navigable
  clojure.lang.IPersistentMap
  (nav [_ k v]
    (cond
      (and (map? v) (:assay/fn v)) (nav-frame v)
      (and (= k :assay/stack) (sequential? v)) (mapv nav-frame v)
      :else v)))
...
(extend-protocol p/Datafiable
  ...
  (datafy [v]
    (let [m (meta v)]
      {:assay/var (symbol (str (ns-name (:ns m))) (str (:name m)))
       :assay/file (:file m)
       :assay/line (:line m)
       :assay/arglists (:arglists m)
       :assay/doc (:doc m)})))
