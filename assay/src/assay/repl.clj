(ns assay.repl
  ...
  whose whole purpose is to be interactive. `assay.capture` and
  `assay.query` stay pure and are what you use in code.
...
(defn summary
  ...
  ([snap]
   (cond-> (assoc (query/summary (:assay/observations snap)
     ...
     ;; If the ring dropped anything, every :assay/n below is a count over
