(ns assay.code
  ...
  assay.measure, separated by blast radius:
  ...
  time — by using information already in hand rather than adding surface:
    ...
    :assay.note/taken / :refused     ; SBCL prints the path taken beside
...
(def ^:private kind-order
  [:assay.note/reflection :assay.note/megamorphic
   :assay.note/boxed-math :assay.note/boxed-body])
...
;; :assay.severity/warning, the only value in the codebase, which makes the
...
(def ^:private severity-of
  {:assay.note/reflection  :assay.severity/warning
   :assay.note/megamorphic :assay.severity/warning
   :assay.note/boxed-math  :assay.severity/note
   :assay.note/boxed-body  :assay.severity/note})

(defn severity
  "Tier for a note code: :assay.severity/warning for per-call member
  resolution, :assay.severity/note for per-op allocation. Unknown codes
  tier as :assay.severity/note rather than silently as warning — a new
  source should not inherit the loudest level by default."
  [note-code]
  (get severity-of note-code :assay.severity/note))
...
(defn muffle!
  ...
  (binding [code/*muffled* #{:assay.note/boxed-math}] ...)."
  ([] (muffle! #{}))
  ([codes]
   (let [s (set codes)]
...
(defn- unmuffled
  ...
  [notes]
  (if (seq *muffled*)
    (vec (remove #(contains? *muffled* (:assay.note/code %)) notes))
    (vec notes)))
...
(defn report
  ...
  needs: assay.jvmci derives notes from the JIT's profile rather than from
  ...
  [notes]
  (vec (sort-by #(kind-rank (:assay.note/code %)) (unmuffled notes))))
...
(def ^:private code-of
  {"Reflection" :assay.note/reflection
   "Boxed math" :assay.note/boxed-math})
      ...
      (when (seq all)
        ...
        #:assay.note{:taken (str method "(" (str/join "," taken) ")")
                    :refused (vec (for [o all
                                        :when (and (not (:assay.overload/taken? o))
                                                   (:assay.overload/primitive? o)
                                                   (= (count (:assay.overload/params o)) (count taken)))]
                                    (str method "(" (str/join "," (:assay.overload/params o))
                                         ")->" (:assay.overload/returns o))))}))))
...
(defn notes*
  ...
  *warn-on-reflection* and *unchecked-math*, capture *err*, parse. Clojure
  ...
  (let [w (java.io.StringWriter.)
        tmp (create-ns (gensym "assay.code.probe"))]
    (try
...
;; `assay.explain/explain` is a different thing entirely — the four-oracle
...
(defn print-notes
  ...
  and the cost number. assay authors none of this text; it passes through
  ...
  (let [ns (if (sequential? ns) ns [ns])]
    (with-out-str
      (doseq [n ns]
        ...
        (printf "%s[%s]: %s\n"
                (name (or (:assay.note/severity n) (severity (:assay.note/code n))))
                (name (:assay.note/code n))
                (:assay.note/message n))
        ...
        (let [{:assay/keys [file line col]} (:assay.note/span n)]
          (when (or file line)
            (printf "  --> %s:%s:%s\n" (or file "<form>") line col)))
        (when-let [e (:assay.note/emitted n)] (printf "   = emitted: %s\n" e))
        (when-let [t (:assay.note/taken n)] (printf "   = took:    %s\n" t))
        (when-let [r (seq (:assay.note/refused n))]
          (printf "   = refused: %s\n" (str/join ", " r)))
        (when-let [v (:assay.note/var n)] (printf "   = var: %s\n" v))
        (println)))))
...
(defn resolve!
  ...
  (or (try (requiring-resolve sym)
           (catch Exception e
             (throw (ex-info (str "not on the classpath: " sym) {:sym sym} e))))
      (throw (ex-info (str "not on the classpath: " sym) {:sym sym}))))
...
(defmacro java
  ...
  [form]
  `((assay.code/resolve! 'clj-java-decompiler.core/decompile-form) {} '~form))

(defmacro bytecode
  "Rung 3 — JVM bytecode."
  [form]
  `((assay.code/resolve! 'clj-java-decompiler.core/decompile-form) {:decompiler :bytecode} '~form))
...
(defn emitted-signature
  ...
  (let [obj (if (var? f) @f f)]
    (->> (.getDeclaredMethods ^Class (class obj))
         ...
         (map (fn [^java.lang.reflect.Method m]
                #:assay.types{:method (.getName m)
                             :params (mapv tname (.getParameterTypes m))
                             :returns (tname (.getReturnType m))}))
         (sort-by :assay.types/method)
         vec)))
   ...
   (let [f (eval form)
         ...
         unresolved-of (fn [sig]
                         (cond-> (vec (keep-indexed #(when (obj? %2) %1)
                                                    (:assay.types/params sig)))
                           (obj? (:assay.types/returns sig)) (conj :return)))
         ...
         candidates (filter #(#{"invokeStatic" "invokePrim"} (:assay.types/method %)) sigs)
         main (or (first (sort-by #(- (count (unresolved-of %)))
                                  (seq (or (seq candidates) sigs))))
                  (first sigs))
         params (:assay.types/params main)
         returns (:assay.types/returns main)
         unresolved (unresolved-of main)
         ret (when args (apply f args))]
     (cond-> #:assay.types{:emitted sigs
                          ...
                          :arities (count (distinct (map (comp count :assay.types/params) sigs)))
                          :params params
                          :returns returns
                          :unresolved unresolved
                          :verdict (cond (empty? unresolved) :assay.types/resolved
                                         (= (count unresolved)
                                            (inc (count params))) :assay.types/all-boxed
                                         :else :assay.types/partial)}
       args (assoc :assay.types/actual (mapv #(tname (class %)) args)
                   :assay.types/actual-returns (tname (class ret)))))))
